...
RUN GOPROXY=${GOPROXY} CGO_ENABLED=0 go build -ldflags "-w -s -X github.com/linuxsuren/api-testing/pkg/version.version=${VERSION}\
    -X github.com/linuxsuren/api-testing/pkg/version.date=$(date +%Y-%m-%d)" -o atest-store-mermaid .

FROM ${BASE_IMAGE}

LABEL org.opencontainers.image.source=https://github.com/linuxsuren/atest-ext-store-mermaid
